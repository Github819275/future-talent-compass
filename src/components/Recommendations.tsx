    ...
    >
      <div className="text-center space-y-2">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs font-medium">
          ...
          Phase 5 — Conditional Recommendations
        </div>
        <h2 className="text-2xl font-display font-bold">Decision Intelligence</h2>
      </div>
      ...
      {keyInsight && (
        <motion.div
          ...
          className="glass-card p-6 border-primary/30 glow-blue text-center"
        >
          <div className="flex items-center justify-center gap-2 mb-3">
            <Lightbulb className="w-5 h-5 text-primary" />
            <span className="text-xs font-medium text-primary uppercase tracking-wider">Key Insight</span>
          </div>
        ...
        {recommendations.map((rec, i) => {
          ...
          return (
            <motion.div
              ...
              className={`glass-card p-5 space-y-3 border-l-2 ${cardAccents[i]} ${i === 2 ? "md:col-span-1" : ""}`}
            >
              <div className="flex items-center gap-2">
                <Icon className="w-4 h-4 text-muted-foreground" />
                <h3 className="text-sm font-display font-semibold text-foreground">{rec.horizon}</h3>
              </div>
              <div className="px-3 py-1.5 rounded bg-primary/10">
              ...
              <div className="pt-2 border-t border-border/30">
                <div className="flex items-start gap-1.5">
                  ...
                  <p className="text-[11px] text-amber">{rec.risk}</p>
                </div>
              </div>
              {rec.developmentPlan && (
                <div className="pt-2 border-t border-border/30">
                  <p className="text-[10px] font-medium text-muted-foreground mb-1">Development Plan:</p>
      ...
      {/* Devil's Advocate */}
      {devilsAdvocate && (
        <div className="glass-card p-5 border-l-2 border-l-danger">
